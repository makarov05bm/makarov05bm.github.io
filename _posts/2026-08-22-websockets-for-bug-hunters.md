...
In this blog, I aim to provide bug hunters and pentesters with all they need to know to start hacking applications that utilize WebSockets, as it's a huge attack surface that's usually goes under the radar in engagements. This blogs starts off by clearing the foundations that you need to easily understand attacks related to WS, because immediately hopping into the bugs and exploits will not make much sense, especially if you are the type of hacker who is not into checklists. Also, I built a lab with most of the bug explained in this blog, so you can get your hands dirty, not just another boring blog post.
